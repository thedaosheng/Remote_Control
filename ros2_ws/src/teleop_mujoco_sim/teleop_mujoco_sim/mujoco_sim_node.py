...
class MujocoSimNode(Node):
    ...
    def run(self):
        ...
        # Settling: 让物理稳定 (2000步 = 4秒，确保右臂收敛到 home)
        # 右臂是力矩执行器，用关节空间 PD + 重力补偿保持 home
        self.get_logger().info('物体 settling (4s, 右臂 PD 收敛)...')
        rj1_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_JOINT, 'right_joint1')
        arm_home = np.array(ARM_HOME_RIGHT)
        for step in range(2000):
            data.ctrl[CTRL_LEFT_ARM:CTRL_LEFT_ARM + 6] = ARM_HOME_LEFT
            data.ctrl[CTRL_GRIPPER] = GRIPPER_CLOSE
            # 右臂: 关节空间 PD 控制 + 重力补偿
            if rj1_id >= 0:
                qpos_off = model.jnt_qposadr[rj1_id]
                qvel_off = model.jnt_dofadr[rj1_id]
                q = data.qpos[qpos_off:qpos_off + 6]
                qd = data.qvel[qvel_off:qvel_off + 6]
                # PD: tau = Kp*(q_des-q) - Kd*qd + gravity
                tau = 200.0 * (arm_home - q) - 20.0 * qd + data.qfrc_bias[qvel_off:qvel_off + 6]
