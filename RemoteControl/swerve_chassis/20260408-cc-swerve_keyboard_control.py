...
# Touch 力反馈笔的 libHD.so 依赖 libncurses.so.5 / libtinfo.so.5
# Ubuntu 22.04 只有 v6，~/teleop/lib/ 放了兼容软链接
# LD_LIBRARY_PATH 必须在进程启动前设置，Python 运行时设太晚
# 方案：检测如果 LD_LIBRARY_PATH 未包含兼容库目录，用 execv 重启自身
_LIB_COMPAT = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'lib')
...
def main():
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        while viewer.is_running() and not exit_request:
            ...
            touch.update()                                                  # 右臂+夹爪 → ctrl[18:24]+ctrl[27]
