...
# ==================== ROS 集成钩子 ====================
# livekit_bridge_node 启动时把 publish callback 注入这里, 收到 pose 时被调用
# 形参: (pitch_deg, yaw_deg, roll_deg, pos_xyz_list, quat_xyzw_list, ts)
# 如果是 None (老脚本独立运行模式), 不调用, 保持纯 UDP 转发行为
ROS_POSE_CALLBACK = None
...
class LiveKitSender:
    ...
    def _on_data_received(self, data_packet: rtc.DataPacket):
        ...
        if msg_type == "pose":
            ...
            # ★ ROS 集成钩子: 如果 livekit_bridge_node 注入了 callback,
            # 把同样的 pose 数据 publish 到 ROS topic /vp/head_pose
            # publisher.publish() 是线程安全的, 可以从 asyncio 回调里直接调
            global ROS_POSE_CALLBACK
            if ROS_POSE_CALLBACK is not None:
                try:
                    phys_roll_deg = math.degrees(yaw)  # 数学 yaw(绕Z) = 物理侧倾
                    ROS_POSE_CALLBACK(
                        pitch_deg=motor_pitch,
                        yaw_deg=motor_yaw,
                        roll_deg=phys_roll_deg,
                        pos=pos,
                        quat=quat,
                        ts=ts,
                    )
                except Exception as e:
                    # ROS publish 失败不影响 UDP 主流程
                    print(f"[ROS hook] publish failed: {e}")
